...
//estimatorTypePub(nh.advertise<std_msgs::Bool>("/estimator_type", 1))
{
  ...
  if (indoorMode) {
    localPositionSub = n.subscribe(uav_prefix + "/mocap/UAV0", 1,
                                   &StateEstimatorNode::GetMocapMsg, this);
    visionPosePub = n.advertise<geometry_msgs::PoseStamped>(
        uav_prefix + "/mavros/vision_pose/pose", 1);
  } else {
    localPositionSub = n.subscribe(
        uav_prefix + "/state_estimator/local_position/odom_adjusted", 1,
        &StateEstimatorNode::GetGPSMsg, this);
  }
  statePub = n.advertise<nav_msgs::Odometry>(
      uav_prefix + "/state_estimator/local_position/odom", 1);
  estimatorTypePub =
      n.advertise<std_msgs::Bool>(uav_prefix + "/estimator_type", 1);

  ROS_INFO("Starting state estimator node for UAV %s in %s mode",
           uav_prefix.c_str(), (indoorMode ? "indoor" : "outdoor"));
}
